...
from app.agent.routes_agent import make_router as make_agent_router

async def _agent_execute(sql: str):
    async with db.pool.acquire() as con:
        rows = await con.fetch(sql)
        return [dict(r) for r in rows]


app.include_router(make_agent_router(_agent_execute))
